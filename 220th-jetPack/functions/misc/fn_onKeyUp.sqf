...
if(_toggleAutoHover)exitWith{_engineState call ["autoHoverSwitch"]};

if(!isTouchingGround _man)
then{[_man] call RJET_fnc_brakeSound;};

if(_key isEqualTo 57)exitWith{[_man, "up"]      call RJET_fnc_thrustStop;};
if(_key isEqualTo 17)exitWith{[_man, "forward"] call RJET_fnc_thrustStop;};
if(_key isEqualTo 32)exitWith{[_man, "right"]   call RJET_fnc_thrustStop;};
if(_key isEqualTo 30)exitWith{[_man, "left"]    call RJET_fnc_thrustStop;};
if(_key isEqualTo 31)exitWith{[_man, "brake"]   call RJET_fnc_thrustStop;};
